...
`;var Je=[["year",31536e3],["month",2592e3],["week",604800],["day",86400],["hour",3600],["minute",60]];function ce(r,e,t=new Date){let i=Math.round((r.getTime()-t.getTime())/1e3);if(!Number.isFinite(i))return"";let o=new Intl.RelativeTimeFormat(e,{numeric:"auto"}),n=Math.abs(i);for(let[a,s]of Je)if(n>=s)return o.format(Math.round(i/s),a);return o.format(0,"second")}var T=class extends Error{constructor(t,i){super(t);this.code=i}code};async function Be(r,e,t){if(r.fetchWithAuth)return r.fetchWithAuth(e,t);let i=r.auth?.data?.access_token??"";return fetch(e,{...t,headers:{Authorization:`Bearer ${i}`}})}function ze(r){if(r.status===413)throw new T("too large","too_large");if(r.status===401||r.status===403)throw new T("forbidden","forbidden");if(!r.ok)throw new T(`${r.status} ${r.statusText}`,"http")}async function et(r,e,t=.85,i){if(!e&&!i||!r.type.startsWith("image/")||r.type==="image/svg+xml"||r.type==="image/gif")return r;let o;try{o=await createImageBitmap(r,{imageOrientation:"from-image"})}catch{return r}let{width:n,height:a}=o,s=0,d=0,c=n,l=a;i&&i>0&&(n/a>i?(c=Math.round(a*i),s=Math.round((n-c)/2)):(l=Math.round(n/i),d=Math.round((a-l)/2)));let p=Math.max(c,l),m=e&&p>e?e/p:1;if(m===1&&c===n&&l===a)return o.close(),r;let _=document.createElement("canvas");_.width=Math.max(1,Math.round(c*m)),_.height=Math.max(1,Math.round(l*m));let y=_.getContext("2d");if(!y)return o.close(),r;y.drawImage(o,s,d,c,l,0,0,_.width,_.height),o.close();let b=r.type==="image/png",g=b?"image/png":"image/jpeg",v=await new Promise(C=>_.toBlob(C,g,b?void 0:t));if(!v)return r;let f=b?r.name:r.name.replace(/\.[a-z0-9]+$/i,"")+".jpg";return new File([v],f,{type:g})}async function tt(r,e){let t=new FormData;t.append("file",e);let i=await Be(r,"/api/image/upload",{method:"POST",body:t});return ze(i),`/api/image/serve/${(await i.json()).id}/original`}async function De(r,e,t){let i=t.trim().replace(/^\/+|\/+$/g,""),o=`${S}media_source/local${i?`/${i}`:""}`,n=e.name.replace(/[^A-Za-z0-9._-]+/g,"_")||"picture.jpg",a=new File([e],`${Date.now()}-${n}`,{type:e.type}),s=new FormData;s.append("media_content_id",o),s.append("file",a);let d=await Be(r,"/api/media_source/local_source/upload",{method:"POST",body:s});return ze(d),(await d.json()).media_content_id}function te(r,e,t,i="memo"){let o=e.type.includes("mp4")||e.type.includes("aac")?"m4a":e.type.includes("ogg")?"ogg":e.type.includes("wav")?"wav":"webm",n=new File([e],`${i}.${o}`,{type:e.type||"audio/webm"});return De(r,n,t)}function ie(){let r=["audio/webm;codecs=opus","audio/webm","audio/mp4","audio/ogg;codecs=opus"],e=window.MediaRecorder;return e?.isTypeSupported?r.find(t=>e.isTypeSupported?.(t))??"":""}async function oe(r,e,t){let i=await et(e,t.maxSize,t.quality,t.cropAspect);return t.target==="media"?De(r,i,t.folder):tt(r,i)}var pe=/^(\s*)[-*+]\s+\[([ xX])\]\s?(.*)$/;function Oe(r){let e=r.split(`
`),t=[],i=[],o=[],n=()=>{i.length&&i.some(s=>s.trim()!=="")&&t.push({type:"markdown",text:i.join(`
`)}),i=[]},a=()=>{o.length&&t.push({type:"checklist",items:o}),o=[]};return e.forEach((s,d)=>{let c=pe.exec(s);c?(n(),o.push({line:d,checked:c[2]!==" ",text:c[3]})):(a(),i.push(s))}),n(),a(),t}function $e(r){return r.split(`
`).some(e=>pe.test(e))}function Ue(r,e,t){let i=r.split(`
`),o=pe.exec(i[e]??"");return o?(i[e]=`${o[1]}- [${t?"x":" "}] ${o[3]}`,i.join(`
`)):r}function ue(r){return/\{\{|\{%/.test(r)}function he(r){let e=r.trim();if(!e)return null;let t=/^(\d{4})-(\d{2})-(\d{2})$/.exec(e);if(t)return new Date(Number(t[1]),Number(t[2])-1,Number(t[3]),23,59,59,999);let i=new Date(e.includes("T")?e:e.replace(" ","T"));return Number.isNaN(i.getTime())?null:i}function ne(r,e=new Date){let t=he(r);return t!==null&&t.getTime()<e.getTime()}var me={yellow:"#fff3a8",green:"#d4f5cd",blue:"#d6ebff",pink:"#ffd9e6",orange:"#ffe0b8",purple:"#e6dcff",grey:"#e9e9ee"};function Ve(r){let e=r.trim().toLowerCase();return e?me[e]??r.trim():null}function je(r){let e=/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.exec(r.trim()),t=255,i=255,o=255;if(e){let a=e[1];a.length===3&&(a=a.split("").map(s=>s+s).join("")),t=Number.parseInt(a.slice(0,2),16),i=Number.parseInt(a.slice(2,4),16),o=Number.parseInt(a.slice(4,6),16)}else{let a=/^rgba?\((\d+)[,\s]+(\d+)[,\s]+(\d+)/i.exec(r.trim());a&&(t=Number(a[1]),i=Number(a[2]),o=Number(a[3]))}return(.299*t+.587*i+.114*o)/255>.55?"#1f1f1f":"#ffffff"}function Ke(r){let e=5381;for(let t=0;t<r.length;t++)e=(e<<5)+e+r.charCodeAt(t)|0;return(e>>>0).toString(36)}var Xe=`
...
<div class="version">Pinboard ${j}</div>`,dt=`
.pages {
  margin-bottom: 16px;
}
.pages-label,
...
`,ae=class extends HTMLElement{_root;_config;_hass;_lang="en";_built=!1;_pageIndex=0;_pageForm;_cardForm;_chips;_previewImg;_preview;_fileInput;_status;_clearButton;_uploadButton;_removePageButton;_moveLeftButton;_moveRightButton;_uploading=!1;_previewToken=0;_selectedMarker=-1;_canvasImg;_previewCard;_dragIndex=-1;_importing=!1;_recorder;_recordTimer;constructor(){super(),this._root=this.attachShadow({mode:"open"})}setConfig(e){this._config={...e};let t=O(this._config).length;this._pageIndex>=t&&(this._pageIndex=t-1),this._render()}set hass(e){this._hass=e;let t=ee(e),i=t!==this._lang;this._lang=t,this._pageForm&&(this._pageForm.hass=e),this._cardForm&&(this._cardForm.hass=e),this._previewCard&&(this._previewCard.hass=e),i?this._render():this._updatePreview()}get hass(){return this._hass}_pages(){return this._config?O(this._config):[{}]}_page(){return this._pages()[this._pageIndex]??{}}_kindOf(e){let t=F(e)||e.kind==="image",i=$(e)||e.kind==="note",o=D(e)||e.kind==="audio";return[t,i,o].filter(Boolean).length>1?"both":o?"audio":i?"note":"image"}_slideCount(e){return J(e.map(Q)).length}_withPage(e,t){let i={...this._config??{type:""}},o=this._pages().map(n=>({...n}));return o[e]=be(t),this._withPages(i,o)}_withPages(e,t){let i={...e};for(let o of B)o!=="title"&&delete i[o];for(let o of fe)delete i[o];if(t.length<=1){let o=be(t[0]??{});for(let n of B){if(n==="title"){o.title&&!i.title&&(i.title=o.title);continue}o[n]!==void 0&&(i[n]=o[n])}}else i.slides=t.map(be);return i}_addPage(e){let t=this._pages().map(i=>({...i}));this._slideCount(t)>=10||(t.push(e==="image"?{}:{kind:e}),this._pageIndex=t.length-1,this._emit(this._withPages(this._config??{type:""},t)))}_removePage(){let e=this._pages().map(t=>({...t}));e.length<=1||(e.splice(this._pageIndex,1),this._pageIndex=Math.min(this._pageIndex,e.length-1),this._emit(this._withPages(this._config??{type:""},e)))}_movePage(e){let t=this._pages().map(n=>({...n})),i=this._pageIndex,o=i+e;o<0||o>=t.length||([t[i],t[o]]=[t[o],t[i]],this._pageIndex=o,this._emit(this._withPages(this._config??{type:""},t)))}_reorder(e,t){let i=this._pages().map(n=>({...n}));if(e<0||e>=i.length||t<0||t>=i.length)return;let[o]=i.splice(e,1);i.splice(t,0,o),this._pageIndex=t,this._emit(this._withPages(this._config??{type:""},i))}async _importFolder(){let e=this._hass,t=this._root.querySelector(".import-folder"),i=this._root.querySelector(".import-status");if(!e||!t||this._importing)return;let o=(s,d)=>h(this._lang,s,d),n=t.value.trim().replace(/^\/+|\/+$/g,""),a=n.startsWith(S)?n:`${S}media_source/local${n?`/${n}`:""}`;this._importing=!0,i&&(i.textContent=o("editor_uploading"),i.classList.remove("error"));try{let d=((await e.callWS({type:"media_source/browse_media",media_content_id:a})).children??[]).filter(p=>p.media_class==="image"||(p.media_content_type??"").startsWith("image/")),c=this._pages().map(p=>({...p})),l=0;for(let p of d){if(this._slideCount(c)>=10)break;c.push({image:p.media_content_id}),l++}l&&(this._pageIndex=c.length-1,this._emit(this._withPages(this._config??{type:""},c))),i&&(i.textContent=l?o("editor_import_done",{count:l}):o("editor_import_none",{folder:n||"/media"}))}catch(s){i&&(i.textContent=`${o("editor_import_failed")}: ${s instanceof Error?s.message:String(s)}`,i.classList.add("error"))}finally{this._importing=!1;let s=this._root.querySelector(".import");s&&(s.disabled=!1)}}_renderAudioEditor(e){let t=this._root.querySelector(".audio-editor");if(!t||(t.classList.toggle("hidden",!e),!e))return;let i=n=>h(this._lang,n),o=(n,a)=>{let s=t.querySelector(n);s&&(s.textContent=a)};o(".audio-label",i("editor_audio")),o(".audio-help",i("editor_audio_help")),o(".record-btn span",i(this._recorder?"editor_stop":"editor_record")),o(".upload-audio span",i("editor_upload_audio")),t.querySelector(".record-btn")?.classList.toggle("active",!!this._recorder),this._updateAudioPreview()}_updateAudioPreview(){let e=this._root.querySelector(".audio-preview");if(!e)return;let t=this._page(),i;if(t.audio_entity&&this._hass){let n=this._hass.states[t.audio_entity];i=n&&n.state!=="unknown"?n.state:""}else i=typeof t.audio=="object"&&t.audio!==null?t.audio.media_content_id:t.audio;if(!i){e.removeAttribute("src");return}if(!i.startsWith(S)){e.getAttribute("src")!==i&&(e.src=i);return}if(!this._hass)return;let o=i;this._hass.callWS({type:"media_source/resolve_media",media_content_id:o,expires:86400}).then(n=>{e.dataset.mediaId!==o&&(e.dataset.mediaId=o,e.src=n.url)}).catch(()=>e.removeAttribute("src"))}_setAudioStatus(e,t=!1){let i=this._root.querySelector(".audio-editor-status");i&&(i.textContent=e,i.classList.toggle("error",t))}async _uploadAudioFile(e,t="memo"){let i=this._hass;if(!i)return;let o=n=>h(this._lang,n);this._setAudioStatus(o("editor_uploading"));try{let n=this._config?.upload_folder??u.upload_folder,a=await te(i,e,n,t),s={...this._page(),audio:a};delete s.kind,this._emit(this._withPage(this._pageIndex,s)),this._setAudioStatus(o("editor_upload_done"))}catch(n){let a=n instanceof T?n.code:"network",s=a==="too_large"?o("editor_upload_too_large"):a==="forbidden"?o("editor_upload_forbidden"):n instanceof Error?n.message:String(n);this._setAudioStatus(`${o("editor_upload_failed")}: ${s}`,!0)}}async _toggleRecord(){let e=(d,c)=>h(this._lang,d,c);if(this._recorder){this._recorder.state!=="inactive"&&this._recorder.stop();return}let t=window.MediaRecorder;if(!t||!navigator.mediaDevices?.getUserMedia){this._setAudioStatus(e("micUnsupported"),!0);return}let i;try{i=await navigator.mediaDevices.getUserMedia({audio:!0})}catch{this._setAudioStatus(e("micDenied"),!0);return}let o=ie(),n=o?new t(i,{mimeType:o}):new t(i),a=[],s=Date.now();n.addEventListener("dataavailable",d=>{d.data.size>0&&a.push(d.data)}),n.addEventListener("stop",()=>{i.getTracks().forEach(d=>d.stop()),window.clearInterval(this._recordTimer),this._recorder=void 0,this._renderAudioEditor(!0),a.length&&this._uploadAudioFile(new Blob(a,{type:n.mimeType||o||"audio/webm"}),`memo-${Date.now()}`)}),this._recorder=n,n.start(),this._renderAudioEditor(!0),this._recordTimer=window.setInterval(()=>{let d=Math.round((Date.now()-s)/1e3);this._setAudioStatus(e("recording",{seconds:d})),d>=180&&this._recorder?.state!=="inactive"&&this._recorder?.stop()},500)}_updatePreviewCard(){let e=this._previewCard;if(!(!e||!this._config))try{e.setConfig({...this._config,type:this._config.type||`custom:${w}`}),this._hass&&(e.hass=this._hass)}catch{}}_selectPage(e){this._pageIndex=e,this._render()}_ensureForm(){customElements.get("ha-form")||window.loadCardHelpers?.().then(e=>{e.createCardElement({type:"entities",entities:[]}).constructor.getConfigElement?.()}).catch(()=>{})}_build(){this._ensureForm(),this._root.innerHTML=`<style>${Fe}${dt}</style>${st}`;let e=o=>this._root.querySelector(o)??void 0;this._pageForm=e(".page-form"),this._cardForm=e(".card-form"),this._chips=e(".chips"),this._preview=e(".preview"),this._previewImg=e(".preview img"),this._fileInput=e(".file"),this._status=e(".status"),this._clearButton=e(".clear"),this._uploadButton=e(".upload"),this._removePageButton=e(".remove-page"),this._moveLeftButton=e(".move-left"),this._moveRightButton=e(".move-right"),this._canvasImg=e(".marker-canvas img"),this._root.querySelector(".marker-canvas")?.addEventListener("click",o=>this._onCanvasClick(o)),this._root.querySelector(".import")?.addEventListener("click",()=>{this._importFolder()}),this._root.querySelector(".import-folder")?.addEventListener("keydown",o=>{o.key==="Enter"&&this._importFolder()});let t=this._root.querySelector(".preview-card");t&&customElements.get(w)&&(this._previewCard=document.createElement(w),this._previewCard&&t.append(this._previewCard)),this._root.querySelector(".play")?.addEventListener("click",()=>this._previewCard?.flip()),this._root.querySelector(".record-btn")?.addEventListener("click",()=>{this._toggleRecord()});let i=this._root.querySelector(".audio-file");this._root.querySelector(".upload-audio")?.addEventListener("click",()=>i?.click()),i?.addEventListener("change",()=>{let o=i.files?.[0];i.value="",o&&this._uploadAudioFile(o)}),this._pageForm?.addEventListener("value-changed",this._onPageValueChanged),this._cardForm?.addEventListener("value-changed",this._onCardValueChanged),this._uploadButton?.addEventListener("click",()=>this._fileInput?.click()),this._fileInput?.addEventListener("change",()=>{let o=this._fileInput?.files?.[0];o&&this._upload(o),this._fileInput&&(this._fileInput.value="")}),this._clearButton?.addEventListener("click",()=>{this._emit(this._withPage(this._pageIndex,{...this._page(),image:void 0,image_entity:void 0}))}),this._removePageButton?.addEventListener("click",()=>this._removePage()),this._moveLeftButton?.addEventListener("click",()=>this._movePage(-1)),this._moveRightButton?.addEventListener("click",()=>this._movePage(1)),this._previewImg?.addEventListener("error",()=>{this._preview?.classList.remove("has-image")}),this._built=!0}_render(){if(!this._config)return;this._built||this._build();let e=(b,g)=>h(this._lang,b,g),t=(b,g)=>{let v=this._root.querySelector(b);v&&(v.textContent=g)};t(".pages-label",e("editor_pages")),t(".pages-help",e("editor_pages_help")),t(".picture-label",e("editor_image")),t(".picture-help",e("editor_image_help")),t(".upload span",e("editor_upload")),t(".clear span",e("editor_clear")),t(".remove-page span",e("editor_remove_page")),t(".move-left span",e("editor_move_left")),t(".move-right span",e("editor_move_right"));let i=this._pages(),o=this._slideCount(i)>=10;this._chips&&(this._chips.replaceChildren(),i.forEach((b,g)=>{let v=this._kindOf(b),f=document.createElement("button");f.type="button",f.className=`chip${g===this._pageIndex?" active":""}`,f.dataset.kind=v;let C=document.createElement("ha-icon");C.setAttribute("icon",v==="note"?"mdi:note-text-outline":v==="audio"?"mdi:microphone-outline":v==="both"?"mdi:image-text":"mdi:image-outline");let L=document.createElement("span");L.textContent=`${g+1} · ${e(`editor_kind_${v}`)}`,f.append(C,L),f.title=e("editor_drag_hint"),f.addEventListener("click",()=>this._selectPage(g)),f.draggable=!0,f.addEventListener("dragstart",k=>{this._dragIndex=g,f.classList.add("dragging"),k.dataTransfer?.setData("text/plain",String(g)),k.dataTransfer&&(k.dataTransfer.effectAllowed="move")}),f.addEventListener("dragend",()=>{this._dragIndex=-1,f.classList.remove("dragging")}),f.addEventListener("dragover",k=>{this._dragIndex<0||this._dragIndex===g||(k.preventDefault(),f.classList.add("drop-target"))}),f.addEventListener("dragleave",()=>f.classList.remove("drop-target")),f.addEventListener("drop",k=>{k.preventDefault(),f.classList.remove("drop-target");let P=this._dragIndex>=0?this._dragIndex:Number(k.dataTransfer?.getData("text/plain"));this._dragIndex=-1,!(!Number.isInteger(P)||P===g)&&this._reorder(P,g)}),this._chips?.append(f)}));let n=this._root.querySelector(".add-row");if(n){n.replaceChildren();for(let b of["image","note","audio"]){let g=document.createElement("button");g.type="button",g.className=`chip add add-${b}`,g.innerHTML='<ha-icon icon="mdi:plus"></ha-icon><span></span>',g.querySelector("span").textContent=e(b==="note"?"editor_add_note":b==="audio"?"editor_add_audio":"editor_add_page"),g.disabled=o,g.addEventListener("click",()=>this._addPage(b)),n.append(g)}}let a=this._root.querySelector(".max-note");a&&(a.textContent=o?e("editor_max_slides"):"");let s=this._root.querySelector(".import-folder");s&&(s.placeholder=e("editor_import"),s.title=e("editor_import_help"),!s.value&&!s.dataset.touched&&(s.value=this._config?.upload_folder??u.upload_folder,s.addEventListener("input",()=>s.dataset.touched="1",{once:!0}))),t(".import span",e("editor_import_button"));let d=this._root.querySelector(".import");d&&(d.disabled=o||this._importing),t(".preview-label",e("editor_preview")),t(".preview-help",e("editor_preview_help")),t(".play span",e("editor_play")),this._updatePreviewCard();let c=this._kindOf(this._page()),l=this._page(),p=c==="image"||F(l),m=c==="audio"||D(l);this._root.querySelector(".picture")?.classList.toggle("hidden",!p),this._renderMarkers(p),this._renderAudioEditor(m),this._removePageButton?.classList.toggle("hidden",i.length<=1),this._moveLeftButton?.classList.toggle("hidden",i.length<=1||this._pageIndex===0),this._moveRightButton?.classList.toggle("hidden",i.length<=1||this._pageIndex>=i.length-1);let _=b=>{let g=`editor_${b.name}_help`,v=e(g);return v===g?"":v},y=b=>b.name==="actions_help"?e("editor_actions_help"):e(`editor_${b.name}`);this._pageForm&&(this._pageForm.hass=this._hass,this._pageForm.schema=this._pageSchema(i.length>1),this._pageForm.data=this._pageData(),this._pageForm.computeLabel=y,this._pageForm.computeHelper=_),this._cardForm&&(this._cardForm.hass=this._hass,this._cardForm.schema=this._cardSchema(),this._cardForm.data=this._cardData(),this._cardForm.computeLabel=y,this._cardForm.computeHelper=_),this._updatePreview()}_pageSchema(e){let t=a=>h(this._lang,a),i=[],o=this._page(),n=this._kindOf(o);return e&&i.push({name:"page_title",selector:{text:{}}}),(n==="audio"||D(o))&&i.push({name:"audio",selector:{text:{}}},{name:"audio_entity",selector:{entity:{filter:[{domain:"input_text"},{domain:"text"}]}}}),(n==="image"||F(o))&&i.push({name:"image",selector:{text:{}}},{name:"image_entity",selector:{entity:{filter:[{domain:"image"},{domain:"camera"},{domain:"person"},{domain:"input_text"},{domain:"text"}]}}}),n==="audio"&&!$(o)||i.push({name:"note",selector:{text:{multiline:!0}}},{name:"note_source",type:"expandable",flatten:!0,icon:"mdi:text-box-edit-outline",title:t("editor_note_source"),expanded:!!this._page().note_entity,schema:[{name:"note_entity",selector:{entity:{}}},{name:"note_attribute",selector:{attribute:{}},context:{filter_entity:"note_entity"}}]},{name:"note_extras",type:"grid",flatten:!0,schema:[{name:"expires",selector:{datetime:{}}},{name:"color",selector:{select:{mode:"dropdown",custom_value:!0,options:[{value:"",label:t("color_none")},...Object.keys(me).map(a=>({value:a,label:t(`color_${a}`)}))]}}}]}),i}_cardSchema(){let e=i=>h(this._lang,i),t=(i,o)=>i.map(n=>({value:n,label:e(`${o}_${n}`)}));return[{name:"title",selector:{text:{}}},{name:"appearance",type:"expandable",flatten:!0,icon:"mdi:palette-outline",title:e("editor_appearance"),expanded:!0,schema:[{name:"appearance_grid",type:"grid",flatten:!0,schema:[{name:"transition",selector:{select:{mode:"dropdown",options:t(I,"transition")}}},{name:"direction",selector:{select:{mode:"dropdown",options:t(N,"direction")}}},{name:"default_side",selector:{select:{mode:"dropdown",options:t(R,"side")}}},{name:"aspect_ratio",selector:{select:{mode:"dropdown",custom_value:!0,options:Ee.map(i=>({value:i,label:i==="auto"?e("ratio_auto"):i}))}}},{name:"image_fit",selector:{select:{mode:"dropdown",options:t(K,"fit")}}},{name:"duration",selector:{number:{min:0,max:5e3,step:50,mode:"box",unit_of_measurement:"ms"}}}]},{name:"appearance_layout",type:"grid",flatten:!0,schema:[{name:"layout",selector:{select:{mode:"dropdown",options:t(W,"layout")}}},{name:"columns",selector:{number:{min:0,max:8,step:1,mode:"box"}}}]},{name:"appearance_toggles",type:"grid",flatten:!0,schema:[{name:"show_title",selector:{boolean:{}}},{name:"show_hint",selector:{boolean:{}}},{name:"show_updated",selector:{boolean:{}}},{name:"show_navigation",selector:{boolean:{}}},{name:"ken_burns",selector:{boolean:{}}}]},{name:"appearance_notes",type:"grid",flatten:!0,schema:[{name:"note_style",selector:{select:{mode:"dropdown",options:t(X,"note_style")}}},{name:"expired_slides",selector:{select:{mode:"dropdown",options:t(q,"expired")}}}]}]},{name:"upload_settings",type:"expandable",flatten:!0,icon:"mdi:folder-image",title:e("editor_upload_settings"),schema:[{name:"upload_target",selector:{select:{mode:"dropdown",options:t(at,"upload_target")}}},{name:"upload_folder",selector:{text:{}}},{name:"upload_max_size",selector:{number:{min:0,max:8e3,step:10,mode:"box",unit_of_measurement:"px"}}},{name:"upload_crop",selector:{boolean:{}}}]},{name:"behaviour",type:"expandable",flatten:!0,icon:"mdi:gesture-tap",title:e("editor_behaviour"),schema:[{name:"behaviour_grid",type:"grid",flatten:!0,schema:[{name:"auto_flip",selector:{number:{min:0,max:3600,step:1,mode:"box",unit_of_measurement:"s"}}},{name:"auto_advance",selector:{number:{min:0,max:3600,step:1,mode:"box",unit_of_measurement:"s"}}},{name:"hover_flip",selector:{boolean:{}}}]},{name:"behaviour_checklist",type:"grid",flatten:!0,schema:[{name:"checklist",selector:{boolean:{}}},{name:"checklist_writeback",selector:{boolean:{}}},{name:"show_camera",selector:{boolean:{}}},{name:"show_record",selector:{boolean:{}}}]},{name:"actions_help",type:"constant",value:""},{name:"hold_action",selector:{ui_action:{actions:Ye,default_action:"none"}}},{name:"double_tap_action",selector:{ui_action:{actions:Ye,default_action:"none"}}}]}]}_pageData(){let e=this._page(),t=typeof e.image=="object"&&e.image!==null?e.image.media_content_id:e.image??"";return{page_title:e.title??"",image:t,image_entity:e.image_entity??"",note:e.note??"",note_entity:e.note_entity??"",note_attribute:e.note_attribute??"",expires:e.expires??"",color:e.color??"",audio:typeof e.audio=="object"&&e.audio!==null?e.audio.media_content_id:e.audio??"",audio_entity:e.audio_entity??""}}_cardData(){let e=this._config??{type:""},t={...u,...ge};for(let[i,o]of Object.entries(e))fe.includes(i)||B.includes(i)&&i!=="title"||(t[i]=o);return t}_onPageValueChanged=e=>{if(e.stopPropagation(),!this._config)return;let t=e.detail.value??{},i={...this._page()};for(let[o,n]of Object.entries(t)){let a=o==="page_title"?"title":o;!B.includes(a)||a==="kind"||a==="markers"||(n==null||n===""?delete i[a]:i[a]=n)}i.kind==="note"&&$(i)&&delete i.kind,i.kind==="image"&&F(i)&&delete i.kind,this._emit(this._withPage(this._pageIndex,i))};_onCardValueChanged=e=>{if(e.stopPropagation(),!this._config)return;let t=e.detail.value??{},i={...this._config};for(let[o,n]of Object.entries(t)){if(o==="type"||fe.includes(o)||B.includes(o)&&o!=="title")continue;let a=o in u?u[o]:ge[o],s=(o in u||o in ge)&&(n===a||typeof n=="object"&&n!==null&&JSON.stringify(n)===JSON.stringify(a));n==null||n===""||s?delete i[o]:i[o]=n}this._emit(i)};_emit(e){let t={};for(let[i,o]of Object.entries(e))o!=null&&o!==""&&(t[i]=o);this._config=t,this._render(),this.dispatchEvent(new CustomEvent("config-changed",{detail:{config:this._config},bubbles:!0,composed:!0}))}async _upload(e){let t=this._hass;if(!t||this._uploading)return;let i=o=>h(this._lang,o);this._uploading=!0,this._setStatus(i("editor_uploading"),!1),this._uploadButton&&(this._uploadButton.disabled=!0);try{let o=this._config??{},n=await oe(t,e,{target:o.upload_target==="media"?"media":"image",folder:o.upload_folder??u.upload_folder,maxSize:o.upload_max_size??u.upload_max_size,cropAspect:o.upload_crop?U(o.aspect_ratio??u.aspect_ratio)??void 0:void 0});this._emit(this._withPage(this._pageIndex,{...this._page(),image:n,image_entity:void 0})),this._setStatus(i("editor_upload_done"),!1)}catch(o){let n=o instanceof T?o.code:"network",a=n==="too_large"?i("editor_upload_too_large"):n==="forbidden"?i("editor_upload_forbidden"):o instanceof Error?o.message:String(o);this._setStatus(`${i("editor_upload_failed")}: ${a}`,!0)}finally{this._uploading=!1,this._uploadButton&&(this._uploadButton.disabled=!1)}}_markers(){let e=this._page().markers;return Array.isArray(e)?e.map(t=>({...t})):[]}_setMarkers(e){let t={...this._page()};e.length?t.markers=e:delete t.markers,this._emit(this._withPage(this._pageIndex,t))}_onCanvasClick(e){if(e.target.closest(".pin"))return;let i=this._canvasImg;if(!i||!i.getAttribute("src"))return;let o=i.getBoundingClientRect();if(!o.width||!o.height)return;let n=Math.round(Math.min(100,Math.max(0,(e.clientX-o.left)/o.width*100))*10)/10,a=Math.round(Math.min(100,Math.max(0,(e.clientY-o.top)/o.height*100))*10)/10,s=this._markers();if(this._selectedMarker>=0&&this._selectedMarker<s.length)s[this._selectedMarker]={...s[this._selectedMarker],x:n,y:a};else{if(s.length>=20)return;s.push({x:n,y:a}),this._selectedMarker=s.length-1}this._setMarkers(s)}_renderMarkers(e){let t=this._root.querySelector(".markers-editor"),i=this._root.querySelector(".marker-canvas .pins"),o=this._root.querySelector(".marker-list");if(!t||!i||!o)return;let n=p=>h(this._lang,p),a=this._page(),s=!!a.image||!!a.image_entity;if(t.classList.toggle("hidden",!e||!s),!e||!s)return;let d=t.querySelector(".markers-label"),c=t.querySelector(".markers-help");d&&(d.textContent=n("editor_markers")),c&&(c.textContent=n("editor_markers_help"));let l=this._markers();if(this._selectedMarker>=l.length&&(this._selectedMarker=-1),i.replaceChildren(),o.replaceChildren(),l.forEach((p,m)=>{let _=document.createElement("button");_.type="button",_.className=`pin${m===this._selectedMarker?" selected":""}`,_.style.left=`${p.x}%`,_.style.top=`${p.y}%`,_.textContent=String(m+1),_.addEventListener("click",f=>{f.stopPropagation(),this._selectedMarker=this._selectedMarker===m?-1:m,this._renderMarkers(!0)}),i.append(_);let y=document.createElement("div");y.className=`marker-row${m===this._selectedMarker?" selected":""}`;let b=document.createElement("button");b.type="button",b.className="marker-number",b.textContent=String(m+1),b.addEventListener("click",()=>{this._selectedMarker=this._selectedMarker===m?-1:m,this._renderMarkers(!0)});let g=(f,C)=>{let L=document.createElement("input");return L.type="text",L.className=`marker-${f}`,L.placeholder=C,L.value=p[f]??"",L.addEventListener("change",()=>{let k=this._markers(),P=L.value.trim();P?k[m]={...k[m],[f]:P}:delete k[m][f],this._setMarkers(k)}),L},v=document.createElement("button");v.type="button",v.className="icon-button",v.title=n("editor_marker_remove"),v.innerHTML='<ha-icon icon="mdi:close"></ha-icon>',v.addEventListener("click",()=>{let f=this._markers();f.splice(m,1),this._selectedMarker=-1,this._setMarkers(f)}),y.append(b,g("label",n("editor_marker_label")),g("icon",n("editor_marker_icon")),g("entity",n("editor_marker_entity")),v),o.append(y)}),!l.length){let p=document.createElement("div");p.className="marker-empty",p.textContent=n("editor_marker_none"),o.append(p)}}_setStatus(e,t){this._status&&(this._status.textContent=e,this._status.classList.toggle("error",t))}_updatePreview(){let e=this._previewImg,t=this._preview;if(!e||!t||!this._config)return;let i=this._page(),o=++this._previewToken,n=d=>{o===this._previewToken&&(d?(e.src=d,t.classList.add("has-image"),this._canvasImg&&(this._canvasImg.src=d)):(e.removeAttribute("src"),t.classList.remove("has-image"),this._canvasImg?.removeAttribute("src")),this._clearButton?.classList.toggle("hidden",!d&&!i.image_entity))},a=i.image;if(i.image_entity&&this._hass){let d=this._hass.states[i.image_entity],c=i.image_entity.split(".")[0];if(c==="input_text"||c==="text")a=d&&d.state!=="unknown"?d.state:"";else{let l=d?.attributes.entity_picture;n(typeof l=="string"?l:"");return}}let s=typeof a=="object"&&a!==null?a.media_content_id:typeof a=="string"&&a.startsWith(S)?a:void 0;if(!s){n(typeof a=="string"?a:"");return}if(!this._hass){n("");return}this._hass.callWS({type:"media_source/resolve_media",media_content_id:s,expires:86400}).then(d=>n(d.url)).catch(()=>n(""))}};function be(r){let e={};for(let t of B){let i=r[t];i!=null&&i!==""&&(e[t]=i)}return e}customElements.get(w)||customElements.define(w,re);customElements.get(z)||customElements.define(z,ae);window.customCards=window.customCards??[];window.customCards.some(r=>r.type===w)||window.customCards.push({type:w,name:ye,description:xe,preview:!0,documentationURL:ke});console.info(`%c Pinboard %c ${j} `,"color: #fff; background: #2c5364; font-weight: 600; border-radius: 4px 0 0 4px; padding: 2px 6px;","color: #2c5364; background: #e6f0f3; font-weight: 500; border-radius: 0 4px 4px 0; padding: 2px 6px;");export{re as PinboardCard,ae as PinboardCardEditor};
